...
* finished looking over comp simp results for curve25519, f51, fmul2,
  ...
  - with my bare metal racket analyzer working on this C code, this
	...
	* i think maybe 40% my code inefficiencies from copying too much
	  ...
	  - for example, if each operand in an expr is an AST, then angr
        ...
        expressions are checked when it should probably be linear in
        the num of insns
  ...
  - overall, the checker analyzed 2267 expressions for comp simp
    safety
  - i hand verified that the 2267 analyzed instructions with 2183 not
    being comp simplifiable and 84 being comp simplifiable are exactly
	...
	* i did not hand analyze all of the 2008 (see below) stack operations
  ...
  - simplifiable stats:
    * there were 84 total exprs
    * simplifiable ops (9/84):
	  - pointer arg has pointer arithmetic done to it before a
        load. the arg here is a pointer to an ECC point 'struct'
	  - this is in a mov from r/m32 to r32 with a displacement
	* not occurring in real binary (1/84):
  ...
  - non-simplifiable stats:
    * there were 2183 total exprs not simplifiable
	
	* push/pop ops (160/2183):
    ...
    * explicit add or sub of rsp/rbp for a load (2008/2183):
