...
module T = struct
  ...
  let add_to_users ~(used : tid) ~(user : tid)
    ...
    let users' = match Tid_map.find users used with
      | Some prev_users -> Set.add prev_users user
      | None -> Tidset.singleton user in
  ...
  let add_to_uses ~user ~used uses : tidset Tid_map.t =
    let uses' = match Tid_map.find uses user with
      | Some prev_used -> Set.add prev_used used
      | None -> Tidset.singleton used in
  ...
  let step numrepeats node data1 data2 =
    (* let teql = Tidset.equal in *)
    (* let get_tidmap_diff = *)
    (*   Tid_map.symmetric_diff ~data_equal:teql in *)
    (* let get_varmap_diff = *)
    (*   Varmap.symmetric_diff ~data_equal:teql in *)
    (* let print_tidset ts = *)
    (*   Tidset.iter ts ~f:(printf "\t%a\n%!" Tid.ppo) in *)
    (* let print_defd to_s (varname, diff) = *)
    (*   match diff with *)
    (*   | `Unequal (l, r) -> *)
    (*      printf "Diff on var %s:\n%!" @@ to_s varname; *)
    (*      printf "Left:\n%!"; *)
    (*      print_tidset l; *)
    (*      printf "Right:\n%!"; *)
    (*      print_tidset r *)
    (*   | _ -> () in *)
    (* let print_all_diffs x y = *)
    ...
    (*            printf "[DepA] repeats (%d) on node: %a\n%!" numrepeats Tid.ppo tid; *)
    (*            printf "[DepA] diff is:\n%!"; *)
    (*            print_all_diffs data1 data2 *)
    (*          else () in *)
    merge data1 data2
end
...
let denote_def (d : def term) (st : t) : t =
    ...
    else Tidset.singleton outer_tid in
  {
    ...
    tid_uses = Tid_map.set st.tid_uses
                 ~key:outer_tid ~data:uses;
    tid_users = users
  }

let denote_elt (e : Blk.elt) (st : t) : t =
  (* let () = printf "[DepA] %a\n%!" Tid.ppo @@ Common.elt_to_tid e in *)
  match e with
