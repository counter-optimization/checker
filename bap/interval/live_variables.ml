...
module IsUsedPass = struct
  module T = struct
    ...
    let used_defining_tid elt = elt.used_defining_tid

    let used elt = elt.used

    let user elt = elt.user

    let user_tid elt = elt.user_tid
  end
...
module Analysis = struct
  let run (sub : Sub.t) : IsUsedPass.UseRel.t =
    ...
    (* let () = printf "In Live_variables.Analysis.run, sub (%s)'s ssa form is:\n%!" *)
    (*            (Sub.name sub) in *)
    (* let () = printf "%a\n%!" Sub.ppo sub_ssa in *)
...
let dependents_up_to_bound tid bound analysis =
  (* set of all use rels whose user_tid is tid *)
  let get_used_tids tid : tid list =
    let has_user = Set.filter analysis ~f:(IsUsedPass.is_user_of_tid tid) in
    (* need to take all of the used tids of the user and put them into into a seq *)
    let used_tids  = Set.to_list has_user
                     |> List.map ~f:IsUsedPass.used_defining_tid
                     |> List.filter ~f:Option.is_some
                     |> List.map ~f:(fun x -> Option.value_exn x) in
    (* let () = Format.printf "tid of def %a depends on tids:\n%!" Tid.pp tid; *)
    (*          List.iter used_tids ~f:(printf "\t%a\n%!" Tid.ppo) in *)
    used_tids
  in
  let rec loop bound q res =
    if bound = 0
    then res
    else
